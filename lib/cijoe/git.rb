class CIJoe
  class Git
    ...
    def update(target_branch = nil)      
      ...
      puts "cd #{@project_path} && git fetch origin && git reset --hard origin/#{branch} && git checkout #{branch}"
      `cd #{@project_path} && git fetch origin && git reset --hard origin/#{branch} && git checkout #{branch}`
    end
